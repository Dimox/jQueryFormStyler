...
function(a){var b=null;"mousewheel"==a.type?b=-1*a.originalEvent.wheelDelta:"DOMMouseScroll"==a.type&&(b=40*a.originalEvent.detail);b&&(a.preventDefault(),e(this).scrollTop(b+e(this).scrollTop()))})}function b(){f.selectSearch&&h.length>f.selectSearchLimit&&(v='<li class="'+w+'"><input class="'+x+' styler" style="width: 100%"  type="search" placeholder="'+f.selectSearchPlaceholder+'" autofocus></li>');i=0;for(len=h.length;i<len;i++){var a="",d="",e=a="",b="",c="";h.eq(i).prop("selected")&&(d="selected sel");
h.eq(i).is(":disabled")&&(d="disabled");h.eq(i).is(":selected:disabled")&&(d="selected sel disabled");void 0!==h.eq(i).attr("class")&&(e=" "+h.eq(i).attr("class"),c=' data-jqfs-class="'+h.eq(i).attr("class")+'"');var g=h.eq(i).data(),l;for(l in g)""!=g[l]&&(a+=" data-"+l+'="'+g[l]+'"');a="<li"+c+a+' class="'+d+e+'">'+h.eq(i).text()+"</li>";h.eq(i).parent().is("optgroup")&&(void 0!==h.eq(i).parent().attr("class")&&(b=" "+h.eq(i).parent().attr("class")),a="<li"+c+' class="'+d+e+" option"+b+'">'+h.eq(i).text()+
"</li>",h.eq(i).is(":first-child")&&(a='<li class="optgroup'+b+'">'+h.eq(i).parent().attr("label")+"</li>"+a));v+=a}}function n(){var m=new r,d=e("<div"+m.id+' class="jq-selectbox jqselect'+m.classes+'" style="display: inline-block; position: relative; z-index:'+f.singleSelectzIndex+'"><div class="jq-selectbox__select"'+m.title+'><div class="jq-selectbox__select-text"></div><div class="jq-selectbox__trigger"><div class="jq-selectbox__trigger-arrow"></div></div></div></div>');a.css({margin:0,padding:0}).wrap('<div class="jq-selectbox-wrapper" style="display: inline-block; position: relative;"></div>').after(d);
var m=a.clone().appendTo("body"),u=m.width();m.remove();u!=a.width()&&a.parent().css("display","block");var m=e("div.jq-selectbox__select",d),k=e("div.jq-selectbox__select-text",d),u=h.filter(":selected");u.length?k.text(u.text()):k.text(h.first().text());b();var c=e('<div class="jq-selectbox__dropdown" style="position: absolute; overflow: auto; overflow-x: hidden"><ul style="list-style: none">'+v+"</ul></div>");d.append(c);var g=e("li",c),l=0;g.each(function(){e(this).css({display:"inline-block",
"white-space":"nowrap"});e(this).width()>l&&(l=e(this).width());e(this).css({display:"block","white-space":"normal"})});d.width(a.outerWidth());l>c.width()&&c.width(l+c.width()-g.width());a.css({position:"absolute",opacity:0,height:d.outerHeight()});var p=g.filter(".selected");1>p.length&&g.first().addClass("selected sel");var q=d.outerHeight();"auto"==c.css("left")&&c.css({left:0});"auto"==c.css("top")&&c.css({top:q});var t=g.outerHeight(),n=c.css("top");c.hide();p.length&&(h.first().text()!=u.text()&&
d.addClass("changed"),d.data("jqfs-class",p.data("jqfs-class")),d.addClass(p.data("jqfs-class")));if(a.is(":disabled"))return d.addClass("disabled"),!1;var y,z=g.children("."+x+"");m.click(function(){a.focus();if(!navigator.userAgent.match(/(iPad|iPhone|iPod)/g)){z.val("");g.show();if(f.selectSmartPositioning){var b=e(window),h=d.offset().top,l=b.height()-q-(h-b.scrollTop()),p=f.selectVisibleOptions,m=6*t,k=t*p;0<p&&6>p&&(m=k);0>l||l<m?(c.height("auto").css({top:"auto",bottom:n}),c.outerHeight()>
h-b.scrollTop()-20&&(c.height(Math.floor((h-b.scrollTop()-20)/t)*t),0<p&&6>p?c.height()>m&&c.height(m):6<p&&c.height()>k&&c.height(k))):l>m&&(c.height("auto").css({bottom:"auto",top:n}),c.outerHeight()>l-20&&(c.height(Math.floor((l-20)/t)*t),0<p&&6>p?c.height()>m&&c.height(m):6<p&&c.height()>k&&c.height(k)))}e("div.jqselect").css({zIndex:f.singleSelectzIndex-1}).removeClass("opened focused");d.css({zIndex:f.singleSelectzIndex});c.is(":hidden")?(e("div.jq-selectbox__dropdown:visible").hide(),c.show(),
d.addClass("opened")):(c.hide(),d.removeClass("opened"));g.filter(".selected").length&&c.scrollTop(c.scrollTop()+g.filter(".selected").position().top-c.innerHeight()/2+t/2);s(c);y=c.height();g.children("."+x+"").focus();return!1}});g.hover(function(){e(this).siblings().removeClass("selected")});var A=g.filter(".selected").text();g.filter(".selected").text();g.filter(":not(.disabled):not(.optgroup):not(."+w+")").click(function(){var b=e(this),l=b.text();if(A!=l){var g=f.selectSearch?b.index()-1:b.index();
b.is(".option")&&(g-=b.prevAll(".optgroup").length);b.addClass("selected sel").siblings().removeClass("selected sel");h.prop("selected",!1).eq(g).prop("selected",!0);A=l;k.text(l);h.first().text()!=l?d.addClass("changed"):d.removeClass("changed");d.data("jqfs-class")&&d.removeClass(d.data("jqfs-class"));d.data("jqfs-class",b.data("jqfs-class"));d.addClass(b.data("jqfs-class"));a.change()}c.hide();d.removeClass("opened")});c.mouseout(function(){e("li.sel",c).addClass("selected")});jQuery.expr[":"].contains=
